...
[System.ComponentModel.DataAnnotations.Schema.Table(name: Infrastructure.TableName.SchemaPerson.EmailType,
                                                    Schema = Infrastructure.SchemaName.SchemaPerson)]
public class EmailType : ID.BaseEntityInt
{
    #region Configuration

    internal class Configuration : IEntityTypeConfiguration<EmailType>
    {
        [Obsolete]
        public void Configure(EntityTypeBuilder<EmailType> builder)
        {
            builder.HasIndex(current => current.CultureId)
                   ...
                   .HasName("IX_person.EmailType.CultureId");
            // **********
            builder.HasIndex(current => current.Name)
                   .IsUnique(unique: true)
                   .HasName("IX_person.EmailType.Name");
            // **********
            builder.HasOne(current => current.Culture)
                    .WithMany(current => current.EmailTypes)
